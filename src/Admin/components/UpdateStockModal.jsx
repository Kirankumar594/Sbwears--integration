...
const UpdateStockModal = ({ isOpen, onClose, product, onStockUpdated }) => {
  ...
  useEffect(() => {
    if (product) {
      ...
      const fetchStockHistory = async () => {
        try {
          const response = await axios.get(`http://localhost:3000/api/admin/products/${product._id}/stock-history`);
  ...
  useEffect(() => {
    if (formData.size && product?.stockBySize) {
      const sizeStock = product.stockBySize.find(s => s.size === formData.size);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.addedStock || isNaN(formData.addedStock) || formData.addedStock <= 0) {
    ...
    try {
      setLoading(true);
      await axios.post(`http://localhost:3000/api/admin/products/${product._id}/stock`, formData);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg w-11/12 md:w-2/3 max-w-4xl p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center border-b pb-4">
          <h2 className="text-lg font-semibold">Update Stock for {product.name}</h2>
              ...
              <tbody>
                {stockHistory.map((history, index) => (
                  <tr key={index} className="border-b">
                    ...
                    <td className="py-2 px-2">{history.previousStock + history.addedStock}</td>
